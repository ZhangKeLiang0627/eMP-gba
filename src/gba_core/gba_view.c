...
void gba_view_init(gba_context_t* ctx, lv_obj_t* par, int mode)
{
    ...
    /* Gesture overlays (top bar + volume bar). Game page: swipes control
     * them, top bar starts hidden; exit goes through ctx->exit_cb. */
    gba_overlay_create(view->root, ctx, NULL, NULL,
                       false /*top_pinned*/, true /*gesture_on*/);
}
